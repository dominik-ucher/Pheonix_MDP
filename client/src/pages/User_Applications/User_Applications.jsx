...
export default function User_Applications() {
  ...
  const handleWithdraw = (index) => {
    ...
    updatedJobs[index] = { ...updatedJobs[index], status: "Withdrawn" };
        ...
        <Table.Row key={index} className="bg-white">
            ...
            <Table.Cell className="flex gap-2">
                <Button 
                    ...
                    style={job.status === "Withdrawn" ? { opacity: 0.5, cursor: "not-allowed" } : {}}
                    disabled={job.status === "Withdrawn"}
                >
                    View Details
                </Button>
                {job.status !== "Withdrawn" && (
                    <Button color="red" onClick={() => handleWithdraw(index)}>
                        Withdraw
                    </Button>
                )}
            </Table.Cell>
